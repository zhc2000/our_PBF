...
void Renderer::SubmitCommandBuffer(VkCommandBuffer& cb,VkSubmitInfo submitinfo,VkFence fence,VkQueue queue)
{
    ...
    if(vkQueueSubmit(queue,1,&submitinfo,fence)!=VK_SUCCESS){
        throw std::runtime_error("failed to submit command buffer! --2");
    }
...
void Renderer::SetParticles(const std::vector<Particle> &ps)
{
    ...
    if (Initialized) {
        ...
        /*for (uint32_t i = 0; i < MAXInFlightRendering; ++i) {
            memcpy(ParticleBuffers[i], &ps, sizeof(&ps));
        }*/
    }
    else {
        particles.assign(ps.begin(), ps.end());
...
Renderer::Renderer(uint32_t w, uint32_t h, bool validation)
{
    ...
    //maxParticles = 100000; // 假设我们设置最大粒子数为100000
    //accumulated_time = 0.0f; // 初始化累积时间为0
...
void Renderer::Updateinit(std::vector<Particle>& particles) {
...
void Renderer::UpdateParticleBuffer(std::vector<Particle>& particles) {
    ...
    for (uint32_t i = 0; i < MAXInFlightRendering; ++i) {
        ...
        CreateBuffer(stagingbuffer, stagingmemory, size,VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
        CreateBuffer(ParticleBuffers[i], ParticleBufferMemory[i], size,VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
...
/*void Renderer::UpdateParticleBuffer(std::vector<Particle>& particles)
{
    ...
    nsobject.numParticles = particles.size();
    nsobject.workgroup_count = WORK_GROUP_COUNT;
    nsobject.hashsize = particles.size() * 2; // 哈希表大小设置为粒子数量的两倍

    // 设置模拟对象中的粒子数量
    simulatingobj.numParticles = particles.size();

    VkDeviceSize size = particles.size() * sizeof(Particle); // 计算粒子数据大小

    // 循环更新每一帧的粒子缓冲区
    for (uint32_t i = 0; i < MAXInFlightRendering; ++i) {
        // 映射内存并将粒子数据复制到临时缓冲区
        void* data;
        vkMapMemory(LDevice, ParticleBufferMemory[i], 0, size, 0, &data);
        memcpy(data, particles.data(), size);

        // 刷新映射的内存，确保数据被写入GPU
        VkMappedMemoryRange memoryRange{};
        memoryRange.sType = VK_STRUCTURE_TYPE_MAPPED_MEMORY_RANGE;
        memoryRange.memory = ParticleBufferMemory[i];
        memoryRange.offset = 0;
        memoryRange.size = size;
        vkFlushMappedMemoryRanges(LDevice, 1, &memoryRange);

        // 由于我们直接更新了GPU的缓冲区，不需要创建和提交复制命令缓冲区
    }
}*/
...
void Renderer::CreateGraphicPipeline()
{
    ...
    //auto fluidvertshadermodule = MakeShaderModule("resources/shaders/spv/your_shader_vert.spv");
    //auto fluidfragshadermodule = MakeShaderModule("resources/shaders/spv/your_shader.spv");
...
std::vector<Particle> Renderer::GetParticlesFromGPU() {
    std::vector<Particle> particlesOnCPU;
    if (Initialized) {
        VkDeviceSize size = particles.size() * sizeof(Particle);

        // 创建一个临时的 staging buffer 用来从 GPU 向 CPU 转移数据
        VkBuffer stagingBuffer;
        VkDeviceMemory stagingMemory;
        CreateBuffer(stagingBuffer, stagingMemory, size,
        ...
        // 创建并记录命令缓冲区，将 GPU 上的粒子数据复制到 staging buffer 中
        VkCommandBuffer cb = CreateCommandBuffer();
        VkBufferCopy region{};
        region.srcOffset = 0;
        region.dstOffset = 0;
        region.size = size;
        vkCmdCopyBuffer(cb, ParticleBuffers[0], stagingBuffer, 1, &region);

        // 提交命令缓冲区并等待所有操作完成
        VkSubmitInfo submitInfo{};
        SubmitCommandBuffer(cb, submitInfo, VK_NULL_HANDLE, GraphicNComputeQueue);
        vkDeviceWaitIdle(LDevice); // Ensure all operations are complete

        // 映射 staging buffer 到 CPU 可见内存，并读取粒子数据
        void* data;
        vkMapMemory(LDevice, stagingMemory, 0, size, 0, &data);
        particlesOnCPU.assign(static_cast<Particle*>(data), static_cast<Particle*>(data) + particles.size());
        vkUnmapMemory(LDevice, stagingMemory);

        // 清理不再需要的 staging buffer 和其分配的内存
        CleanupBuffer(stagingBuffer, stagingMemory, true);
    }
    else {
        throw std::runtime_error("Vulkan not initialized yet!");
    }

    return particlesOnCPU;
}

void Renderer::HandleMouseClick(double screenX, double screenY, std::vector<Particle>& particles) {
    // 将屏幕坐标转换为归一化设备坐标（NDC），即 [-1, 1] 范围内
    float ndcX = (screenX / Width) * 2.0f - 1.0f;
    float ndcY = 1.0f - (screenY / Height) * 2.0f;

    // 使用当前视图和投影矩阵将 NDC 转换回世界坐标
    glm::vec4 worldRayStart(ndcX, ndcY, -1.0f, 1.0f);
    glm::vec4 worldRayEnd(ndcX, ndcY, 1.0f, 1.0f);

    worldRayStart = renderingobj.inv_projection * renderingobj.view * worldRayStart;
    worldRayEnd = renderingobj.inv_projection * renderingobj.view * worldRayEnd;

    worldRayStart /= worldRayStart.w;
    worldRayEnd /= worldRayEnd.w;

    glm::vec3 rayStart(worldRayStart);
    glm::vec3 rayEnd(worldRayEnd);
    glm::vec3 clickPosition = rayStart + (rayEnd - rayStart) * 0.5f; // 取中间点作为点击位置

    ApplyForceAroundPoint(clickPosition,0.1,1,particles);
}
void Renderer::ApplyForceAroundPoint(const glm::vec3& point, float radius, float forceMagnitude, std::vector<Particle>& particles) {
    // 遍历所有粒子，检查哪些粒子在指定的半径范围内
    for (auto& particle : particles) {
        float distance = glm::distance(particle.Location, point);
        if (distance < radius) {
            // 计算从点击点指向粒子的方向向量，并标准化
            glm::vec3 direction = glm::normalize(particle.Location - point);
            // 向粒子施加一个与距离成反比的力，以实现向外推的效果
            particle.Velocity += direction * (forceMagnitude / (distance + 0.001f)); // 加一个小数避免除零
        }
    }


}
/*id Renderer::UpdateParticleBuffer(const std::vector<Particle>&newParticles) {
    // 等待 GPU 完成对当前粒子缓冲区的操作
    vkQueueWaitIdle(GraphicNComputeQueue);

    // 创建一个新的更大的缓冲区
    VkDeviceSize newSize = newParticles.size() * sizeof(Particle);
    VkBuffer newParticleBuffer;
    VkDeviceMemory newParticleBufferMemory;
    CreateBuffer(newParticleBuffer, newParticleBufferMemory, newSize,
        VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);

    // 将新粒子数据复制到新的缓冲区
    void* data;
    vkMapMemory(LDevice, newParticleBufferMemory, 0, newSize, 0, &data);
    memcpy(data, newParticles.data(), newSize);
    vkUnmapMemory(LDevice, newParticleBufferMemory);

    // 清理旧的粒子缓冲区
    //CleanupBuffer(ParticleBuffers, ParticleBufferMemory);

    // 更新 ParticleBuffer 和 ParticleBufferMemory
    ParticleBuffers[0] = newParticleBuffer;
    ParticleBufferMemory[0] = newParticleBufferMemory;
